...
| 섹션 | 아이콘 | 조건 |
|------|--------|------|
| 중요 | ⭐ 황금 | `isStarred` — 완료 여부 무관, 항상 중요 섹션 유지 (최대 5개) |
| 할 일 | 📥 | `!isChecked && !isStarred` |
| 완료 | ✅ | `isChecked && !isStarred` — 중요 항목은 제외 |

> 중요 항목이 완료되어도 중요 섹션에서 완료 상태로 표시되며, 완료 섹션으로 이동하지 않음
...
| 배치 시트 내 브레인덤핑 빠른 입력 | `home_screen.dart` — `_PlacementSheet` 상단에 입력 필드 추가, 엔터 시 즉시 목록 반영 |
| 캘린더 셀 배치 끝 시간 수정 | `timebox_calendar_widget.dart` — `endMinute = cellMinute + intervalMin`으로 탭한 셀 포함 |
